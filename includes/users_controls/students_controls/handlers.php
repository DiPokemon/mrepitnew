...
add_action('admin_post_school_student_create', function () {
    ...
    if (!$user_login || !$user_email) {
        wp_redirect(add_query_arg(['page'=>'school-student-add','error'=>rawurlencode('Заполните логин и email')], admin_url('admin.php')));
        exit;
    }
    ...
    update_user_meta($new_id, 'student_phone', sanitize_text_field(wp_unslash($_POST['student_phone'] ?? '')));
    update_user_meta($new_id, 'student_timezone', sanitize_text_field(wp_unslash($_POST['student_timezone'] ?? '')));
    update_user_meta($new_id, 'student_tg_opt_in', isset($_POST['student_tg_opt_in']) ? 'yes' : '');

    // Привязка родителей (если менеджер/админ)
    if (school_admin_can_link_family() && function_exists('carbon_set_user_meta')) {
        ...
        // синхронизация: добавим student в parent_children
...
add_action('admin_post_school_student_update', function () {
    ...
    update_user_meta($user_id, 'student_phone', sanitize_text_field(wp_unslash($_POST['student_phone'] ?? '')));
    update_user_meta($user_id, 'student_timezone', sanitize_text_field(wp_unslash($_POST['student_timezone'] ?? '')));
